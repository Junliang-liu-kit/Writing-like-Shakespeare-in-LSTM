...
# hyperparameters
emb_size = 4 #embedding layer to reduce dimensionality
hidden_size = 32  # size of hidden layer of neurons
seq_length = 64  # number of steps to unroll the RNN for
learning_rate = 5e-2
max_updates = 500000

concat_size = emb_size + hidden_size # hidden state size + emmbedding state size
...
def forward(inputs, targets, memory):
    ...
    xs, wes, zs, cs, hs, os, ps, ys, i, f, o, c_hat = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
    ...
    for t in range(len(inputs)):
        ...
        f[t] = sigmoid(np.dot(Wf, zs[t]) + bf)

        # compute the update gate
        # i_gate = sigmoid (W_i \cdot [h X] + b_i)
        i[t] = sigmoid(np.dot(Wi, zs[t]) + bi)
        ...
        cs[t] = f[t] * cs[t - 1] + i[t] * c_hat[t]

        # output gate
        # o_gate = sigmoid (Wo \cdot [h X] + b_o)
        o[t] = sigmoid(np.dot(Wo, zs[t]) + bo)
    ...
    activations = (xs, wes, zs, cs, hs, os, ps, ys, i, f, o, c_hat)
...
def backward(activations, clipping=True):
    ...
    xs, wes, zs, cs, hs, os, ps, ys, i, f, o, c_hat = activations
    ...
    for t in reversed(range(len(inputs))):
        ...
        dh = np.dot(Why.T, dos) + dhnext   ##hidden
        do_mit = dh * np.tanh(cs[t]) * dsigmoid(o[t])##hidden
        dWo += np.dot(do_mit, zs[t].T)
        dbo += do_mit

        dc = dh * o[t] * (dtanh(np.tanh(cs[t]))) + dcnext
        dcnext = dc * f[t]

        dc_hat = dc * i[t]
        dc_mit = dc_hat * dtanh(c_hat[t])
        dWc += np.dot(dc_mit, zs[t].T)
        dbc += dc_mit

        di = dc * c_hat[t]
        di_mit = di * dsigmoid(i[t])
        dWi += np.dot(di_mit, zs[t].T)
        dbi += di_mit

        df = dc * cs[t - 1]
        df_mit = df * dsigmoid(f[t])
        dWf += np.dot(df_mit, zs[t].T)
        dbf += df_mit

        dwes = np.dot(Wf[:, hidden_size:].T, df_mit) + np.dot(Wi[:, hidden_size:].T, di_mit) + np.dot(
            Wc[:, hidden_size:].T, dc_mit) + np.dot(Wo[:, hidden_size:].T, do_mit)

        dhnext = np.dot(Wf[:, :hidden_size].T, df_mit) + np.dot(Wi[:, :hidden_size].T, di_mit) + np.dot(
            Wc[:, :hidden_size].T, dc_mit) + np.dot(Wo[:, :hidden_size].T, do_mit)
...
def sample(memory, seed_ix, n):
    ...
    for t in range(n):
        ...
        zs_samp = np.row_stack((h, x_samp))

        f_samp = sigmoid(np.dot(Wf, zs_samp) + bf)

        i_samp = sigmoid(np.dot(Wi, zs_samp) + bi)

        c_hat_samp = np.tanh(np.dot(Wc, zs_samp) + bc)

        cs_samp = f_samp * c + i_samp * c_hat_samp

        o_samp = sigmoid(np.dot(Wo, zs_samp) + bo)

        hs_samp = o_samp * np.tanh(cs_samp)

        os_samp = np.dot(Why, hs_samp) + by

        h, c = hs_samp, cs_samp

        ps_samp = softmax(os_samp)

        ix = np.random.multinomial(10, ps_samp.ravel())

        max_times = np.amax(ix)
        indexs = np.argwhere(ix == max_times)

        index = random.sample(list(indexs), 1)
...
if option == 'train':
    ...
    while True:
        ...
        n_updates += 1
        if n_updates >= max_updates: # Stop condition
            break
